...
const movieThumbnails: Record<string, string> = {
  // Marvel Movies
...
// Function to update movie thumbnails
async function updateMovieThumbnails() {
  try {
    console.log("Starting thumbnail update process...");
    
    // Get all movies from the database
    const allMovies = await db.select().from(schema.movies);
    console.log(`Found ${allMovies.length} movies in the database.`);
    
    let updatedCount = 0;
    
    // Update each movie's thumbnail if a better one exists
    for (const movie of allMovies) {
      if (movieThumbnails[movie.title]) {
        await db.update(schema.movies)
          .set({ posterUrl: movieThumbnails[movie.title] })
          .where(eq(schema.movies.id, movie.id));
        
        updatedCount++;
        console.log(`Updated thumbnail for: ${movie.title}`);
      }
    }
    
    console.log(`Updated thumbnails for ${updatedCount} movies.`);
